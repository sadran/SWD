...
def get_image_path(idx, prefix, training=True, relative_path=True):
    return get_swd_info_path(idx, prefix, 'image_2', '.png', training,
                               relative_path)


def get_label_path(idx, prefix, training=True, relative_path=True):
    return get_swd_info_path(idx, prefix, 'label_2', '.txt', training,
                               relative_path)


def get_velodyne_path(idx, prefix, training=True, relative_path=True):
    return get_swd_info_path(idx, prefix, 'velodyne', '.bin', training,
                               relative_path)


def get_calib_path(idx, prefix, training=True, relative_path=True):
    return get_swd_info_path(idx, prefix, 'calib', '.txt', training,
                               relative_path)
...
def kitti_result_line(result_dict, precision=4):
    ...
    for key, val in res_dict.items():
        if key == 'name':
            res_line.append(val)
        elif key in ['truncated', 'alpha', 'rotation_x', 'score']:
            if val is None:
...
def get_label_anno(label_path):
    ...
    annotations['rotation_x'] = np.array(
