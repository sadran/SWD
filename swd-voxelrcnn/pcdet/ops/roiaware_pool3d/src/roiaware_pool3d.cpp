...
inline void lidar_to_local_coords_cpu(float shift_y, float shift_z, float rot_angle, float &local_y, float &local_z){
    float cosa = cos(-rot_angle), sina = sin(-rot_angle);
    local_y = shift_y * cosa + shift_z * (-sina);
    local_z = shift_y * sina + shift_z * cosa;
}


inline int check_pt_in_box3d_cpu(const float *pt, const float *box3d, float &local_y, float &local_z){
    // param pt: (x, y, z)
    // param box3d: [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    const float MARGIN = 1e-2;
    float x = pt[0], y = pt[1], z = pt[2];
    float cx = box3d[0], cy = box3d[1], cz = box3d[2];
    float dx = box3d[3], dy = box3d[4], dz = box3d[5], rx = box3d[6];

    if (fabsf(x - cx) > dx / 2.0) return 0;
    lidar_to_local_coords_cpu(y - cy, z - cz, rx, local_y, local_z);
    float in_flag = (fabs(local_y) < dy / 2.0 + MARGIN) & (fabs(local_z) < dz / 2.0 + MARGIN);
    return in_flag;
}
...
//    CHECK_CONTIGUOUS(pts_indices_tensor);
    ...
    float local_y = 0, local_z = 0;
    for (int i = 0; i < boxes_num; i++){
        for (int j = 0; j < pts_num; j++){
            int cur_in_flag = check_pt_in_box3d_cpu(pts + j * 3, boxes + i * 7, local_y, local_z);
