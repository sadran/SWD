...
class ATSSTargetAssigner(object):
    ...
    """
    def __init__(self, topk, box_coder, match_length=False):
        self.topk = topk
    ...
    def assign_targets_single(self, anchors, gt_boxes, gt_classes):
        ...
        xyz_local = common_utils.rotate_points_along_x(
            xyz_local[:, None, :], -gt_boxes_of_each_anchor[:, 6]
        ).squeeze(dim=1)
        yz_local = xyz_local[:, 1:3]
        wh = gt_boxes_of_each_anchor[:, 4:6][:, [1, 0]]  # bugfixed: w ==> y, l ==> x in local coords
        is_in_gt = ((yz_local <= wh / 2) & (yz_local >= -wh / 2)).all(dim=-1).view(-1, num_gt)  # (K, M)
