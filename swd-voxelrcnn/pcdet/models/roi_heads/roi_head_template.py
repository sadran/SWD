...
class RoIHeadTemplate(nn.Module):
    ...
    def assign_targets(self, batch_dict):
        ...
        roi_center = rois[:, :, 0:3]
        roi_rx = rois[:, :, 6] % (2 * np.pi)
        gt_of_rois[:, :, 0:3] = gt_of_rois[:, :, 0:3] - roi_center
        gt_of_rois[:, :, 6] = gt_of_rois[:, :, 6] - roi_rx

        # transfer LiDAR coords to local coords
        gt_of_rois = common_utils.rotate_points_along_x(
            points=gt_of_rois.view(-1, 1, gt_of_rois.shape[-1]), angle=-roi_rx.view(-1)
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS == 'smooth-l1':
            ...
            if loss_cfgs.CORNER_LOSS_REGULARIZATION and fg_sum > 0:
                ...
                roi_rx = fg_roi_boxes3d[:, :, 6].view(-1)
                ...
                rcnn_boxes3d = common_utils.rotate_points_along_x(
                    rcnn_boxes3d.unsqueeze(dim=1), roi_rx
                ).squeeze(dim=1)
    ...
    def generate_predicted_boxes(self, batch_size, rois, cls_preds, box_preds):
        ...
        batch_box_preds = common_utils.rotate_points_along_x(
